...
`/rfq/` сначала разбирает описательные требования (тип, qty, мощность, напряжение, фаза, топология, батарея, backup), потом сверяет каталог. Нет модели в каталоге: строка остаётся, Needs verification / No catalog match. Модели и недостающие характеристики не выдумываются.
